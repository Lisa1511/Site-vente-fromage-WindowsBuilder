...
public class PageVache {

	private JFrame frmVache;
	...
	private List<Fromage> fromagesVache = new ArrayList<Fromage>();
	 ...
	 */
	public PageVache(Panier panier) {
		initialize(panier);
		frmVache.setVisible(true);
	}
	...
	private void initialize(Panier panier) {
		...
		frmVache = new JFrame();
		frmVache.setIconImage(Toolkit.getDefaultToolkit().getImage(PageVache.class.getResource("logo.png")));
		frmVache.setTitle("Vache");
		frmVache.setExtendedState(JFrame.MAXIMIZED_BOTH);
		frmVache.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmVache.getContentPane().setLayout(new BorderLayout(25, 10));
		
		//NORTH
		
		JPanel panel_nord = new JPanel();
		frmVache.getContentPane().add(panel_nord, BorderLayout.NORTH);
		...
		frmVache.getContentPane().add(scrollMenu, BorderLayout.WEST);
		...
		//searchButton.setIcon(new ImageIcon(PageVache.class.getResource("/Image/icons/3936854_creanimasi_pencarian_searching_search.png")));
		...
		float PRIX_MAX = fromagesVache.get(0).getArticles().get(0).getPrixTTC();
		float PRIX_MIN = fromagesVache.get(0).getArticles().get(0).getPrixTTC();
		for (Fromage f : fromagesVache) {
		...
		frmVache.getContentPane().add(scrollFromages, BorderLayout.CENTER);
		...
		frmVache.getContentPane().add(footer, BorderLayout.SOUTH);
	...
	public void genererCartesFromage(JPanel p, int prixMax, String searchedWord, ArrayList<TypeOccasion> wantedOccasions, ArrayList<TypePate> wantedPate, Panier panier){
		...
		for (int i = 0; i < fromagesVache.size(); i ++) {
			final Article fArticle = fromagesVache.get(i).getFArticle();
			if (isFromageWanted(fromagesVache.get(i), prixMax, searchedWord, wantedOccasions, wantedPate)) {		
				nbCases = nbCases + 1;
				...
				ImageIcon iconFromage = new ImageIcon(PageVache.class.getResource("/fromage/fromage" + String.valueOf(i%9) + ".jpg"));
				...
				JLabel nameFromage = new JLabel("<html>" + fromagesVache.get(i).getDesignation() + "</html>");
				...
				buttonAddFromage.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						new PopUpWindowAjouterPanier(fromagesVache.get(i2), panier, "/fromage/fromage"+String.valueOf(i2%9)+".jpg");
					}
				});
				...
				tFieldPrice.addMouseListener(new MouseAdapter() {
					...
					public void mouseEntered(MouseEvent e) {
						...
						imgFromage.setText("<html>" + fromagesVache.get(i2).getDescription() + "</html>");
				...
				panelInfoFromage.add(panelNote);
					for (int j = 0; j < fromagesVache.get(i).getNote(); j++) {
						JLabel labelStarIcon = new JLabel();
						ImageIcon iconNoteJ = new ImageIcon(PageVache.class.getResource("/starIcon/star"+ (j + 1) +".png"));
		...
		for (int i = nbCases; i < (4)*2; i++) {
			...
			JLabel imgFromage = new JLabel("");
			imgFromage.setIcon(new ImageIcon(PageVache.class.getResource("/Image/filler.png")));
