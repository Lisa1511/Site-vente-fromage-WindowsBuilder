...
public class PageBrebis {

	private JFrame frmBrebis;
	...
	private List<Fromage> fromagesBrebis = new ArrayList<Fromage>();
	 ...
	 */
	public PageBrebis(Panier panier) {
		initialize(panier);
		frmBrebis.setVisible(true);
	}
	...
	private void initialize(Panier panier) {
		...
		for (Fromage f : fromages) {
			if (f.getTypeFromage() == TypeLait.BREBIS) {
				fromagesBrebis.add(f);
			}
		}
		
		frmBrebis = new JFrame();
		frmBrebis.setIconImage(Toolkit.getDefaultToolkit().getImage(PageBrebis.class.getResource("logo.png")));
		frmBrebis.setTitle("Brebis");
		frmBrebis.setExtendedState(JFrame.MAXIMIZED_BOTH);
		frmBrebis.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmBrebis.getContentPane().setLayout(new BorderLayout(25, 10));
		
		//NORTH
		
		JPanel panel_nord = new JPanel();
		frmBrebis.getContentPane().add(panel_nord, BorderLayout.NORTH);
		...
		frmBrebis.getContentPane().add(scrollMenu, BorderLayout.WEST);
		...
		//searchButton.setIcon(new ImageIcon(PageBrebis.class.getResource("/Image/icons/3936854_creanimasi_pencarian_searching_search.png")));
		...
		float PRIX_MAX = fromagesBrebis.get(0).getArticles().get(0).getPrixTTC();
		float PRIX_MIN = fromagesBrebis.get(0).getArticles().get(0).getPrixTTC();
		for (Fromage f : fromagesBrebis) {
		...
		frmBrebis.getContentPane().add(scrollFromages, BorderLayout.CENTER);
		...
		frmBrebis.getContentPane().add(footer, BorderLayout.SOUTH);
	...
	public void genererCartesFromage(JPanel p, int prixMax, String searchedWord, ArrayList<TypeOccasion> wantedOccasions, ArrayList<TypePate> wantedPate, Panier panier){
		...
		for (int i = 0; i < fromagesBrebis.size(); i ++) {
			final Article fArticle = fromagesBrebis.get(i).getFArticle();
			if (isFromageWanted(fromagesBrebis.get(i), prixMax, searchedWord, wantedOccasions, wantedPate)) {		
				nbCases = nbCases + 1;
				...
				ImageIcon iconFromage = new ImageIcon(PageBrebis.class.getResource("/fromage/fromage"+String.valueOf(i%9)+".jpg"));
				...
				JLabel nameFromage = new JLabel("<html>" + fromagesBrebis.get(i).getDesignation() + "</html>");
				...
				buttonAddFromage.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						new PopUpWindowAjouterPanier(fromagesBrebis.get(i2), panier, "/fromage/fromage"+String.valueOf(i2%9)+".jpg");
					}
				});
				...
				tFieldPrice.addMouseListener(new MouseAdapter() {
					...
					public void mouseEntered(MouseEvent e) {
						...
						imgFromage.setText("<html>" + fromagesBrebis.get(i2).getDescription() + "</html>");
				...
				panelInfoFromage.add(panelNote);
					for (int j = 0; j < fromagesBrebis.get(i).getNote(); j++) {
						JLabel labelStarIcon = new JLabel();
						ImageIcon iconNoteJ = new ImageIcon(PageBrebis.class.getResource("/starIcon/star"+ (j + 1) +".png"));
		...
		for (int i = nbCases; i < (4)*2; i++) {
			...
			JLabel imgFromage = new JLabel("");
			imgFromage.setIcon(new ImageIcon(PageBrebis.class.getResource("/Image/filler.png")));
