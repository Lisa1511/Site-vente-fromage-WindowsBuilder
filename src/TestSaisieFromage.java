...
public class TestSaisieFromage {
	...
	public void testSaisieFromageEtArticlesALaCoupe() {
		SaisieFromage uneSaisie = new SaisieFromage("Brie de Melun", 
				"Plus petit que son grand frere «le Brie de Meaux», il a cependant plus de caractere. Ce fromage au lait cru de vache vous offrira une pâte "
				...
				+"Le Brie de Melun vous proposera une saveur tres fruitee avec un leger goût de noisette accompagnee d’une odeur du terroir qui le rendra "
	...
	public void testSaisieFromageEtArticleALunite() {
		SaisieFromage uneSaisie  = new SaisieFromage("Brique de Brebis Fermiere", 
				...
				+"Lors de sa degustation vous decouvrirez des saveurs fruitees, legerement salees avec des arômes de brebis, de noisette et de lait chaud. "  
				+"Une authenticite qui revele la diversite des pâturages du Tarn grâce à son herbage riche et floral. "
				+ "Un vrai produit qui doit sa beaute simplement au fermier artisan. "
				+ "Elle est fondante, douce et persistante, prend du caractere sans agressivite en s’affinant avec un petit goût de noisette.", 
				TypeVente.A_L_UNITE,new String[]{""} ,new float[]{7.89F});
		Fromage f = uneSaisie.builderFromage();
		assertEquals("Brique de Brebis Fermiere", f.getDesignation());
	...
	public void testSaisieFromageEtArticlesEntierOuMoitie() {
		SaisieFromage uneSaisie = new SaisieFromage("Brin d'Amour Corse", 
				"Ce fromage au lait cru de brebis à pâte molle à croûte fleurie, propose un caractere corse qui s'exprime par sa croûte recouverte d'herbes, "
				...
				+"C'est tout le caractere du fromage corse vous faisant voyager qui s'exprime dans ce mariage tres heureux de lait de brebis et d’aromates. "
	...
	public void testSaisieFromageEtArticlesALunitePlusieursChoix() {
		SaisieFromage uneSaisie = new SaisieFromage("Cabris Fermier", 
				"Produit par nos soins sous le nom de l'EARL Chemin Fleury, nous respectons le savoir faire de cette grande region, tout en se distinguant "
						+"par une texture et un goût qui lui sont bien specifiques. Au lait cru de chevre, nos fromages presentent une croûte fine salee au sel blanc "
						+"ou cendre renfermant une pâte souple, tendre et homogene de couleur blanche. "
						+"Leurs doux goûts de chevre uniques raviront vos papilles et celles de vos convives.", 
		...
		assertEquals(2, f.nombreArticles());
		assertEquals("Moelleux cendre - Jeune et doux", f.getArticles().get(0).getCle());
		assertEquals(3.39F, f.getArticles().get(0).getPrixTTC(), 0F);
		assertEquals("Moelleux blanc - Jeune et doux", f.getArticles().get(1).getCle());
